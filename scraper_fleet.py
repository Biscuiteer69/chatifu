...
TARGETS: dict[str, dict] = {
    ...
    # Siemens Healthineers document library — plain JSON, no auth, own host. Only the
    # DIAGNOSTICS half is reachable (imaging operator manuals are owner-gated and return zero
    # for every catalog), so the resolver orders diagnostics devices first rather than burning
    # batches on guaranteed misses.
    "siemens": {
        "enabled": True, "rank": 4, "host": "doclib.siemens-healthineers.com",
        "cmd": [PY, "-m", "resolvers.siemens_resolver", "--batch", "60"],
        "batch_re": re.compile(r"Resolving (\d+) Siemens devices"),
        "sleep_between": 30, "idle_sleep": 12 * 3600, "batch_timeout": 2400,
    },
    ...
    # RE-ENABLED 2026-09-02. Disabled 2026-08-02 after 300 rows / ZERO found / 22 WAF blocks,
    # with the note "establish OFFLINE which catalog family the portal indexes". Answer: the
    # MODEL. GUDID's Alcon catalog is model + diopter (SA6AT4.320); the portal only knows
    # SA6AT4. The tenant now searches by model (search_key="model" in qarad_tenants.py) and
    # writes the hit to every catalog of that model — the first 3 requests resolved 274
    # devices. --batch counts MODELS: ~40 models is ~40-80 requests and several thousand
    # devices, so this is far cheaper per device than any other tenant.
    "alcon": {
        "enabled": True, "rank": 11, "host": "qarad",
        "cmd": [PY, "-m", "resolvers.qarad_tenants", "--tenant", "alcon", "--batch", "40"],
        "batch_re": re.compile(r"Resolving (\d+) Alcon devices"),
        "sleep_between": 360, "idle_sleep": 12 * 3600, "batch_timeout": 2400,
        "max_backoff": 4 * 3600,
    },
    "coopersurgical": {
